...
| **Image-text inconsistency recall** | weak-impl recall ≈ 0.26 | **mmdataset-v3: 0.836 (27b) / 0.878 (plus)**, F1 0.89~0.92, FPR≈0.10 |

> Caveat on the numbers: the weak-impl recall≈0.26 is an early internal snapshot (no Step 4 re-review, weak prompt); the right column is the measured result of the four-stage strong pipeline on **mmdataset-v3** (full cross-model table in [10 · Metrics & Evaluation](10-量化指标与评测.en.md)). The two are different dataset snapshots and not a strictly controlled comparison, but they show the order-of-magnitude lift from the redesign. The strong pipeline costs more tokens and latency, with longer prompts.

**One-line interview version**: "I refactored image-text consistency detection from 'throw the image at the VLM once' into a **four-stage pipeline + a text-only re-review safety net**, and changed the main detection prompt from loose examples into **forced item-by-item reconciliation** (image observation checklist + claim reconciliation table). On mmdataset-v3, image-text inconsistency recall reaches 0.84 (27B local) ~ 0.88 (plus cloud), F1 0.89~0.92, precision 0.95+, FPR≈0.10, while 'the re-review only flips false→true' preserved precision (the weak implementation reached only ~0.26 recall)."
